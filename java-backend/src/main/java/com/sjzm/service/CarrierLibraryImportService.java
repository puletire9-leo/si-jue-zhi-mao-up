 ...
 * 运营商库Excel导入服务
 * <p>
 * 对齐 Python carrier_library 的批量导入逻辑：
 * 1. 支持 .xlsx 和 .xls 格式
 * 2. 检查数据库中已存在的SKU
 * 3. 支持跳过/覆盖模式
 * 4. 返回详细的导入结果
...
public class CarrierLibraryImportService {
    ...
    public Map<String, Object> importFromExcel(MultipartFile file, boolean overwrite) {
        log.info("开始导入运营商Excel文件: filename={}, size={}, overwrite={}",
    ...
    private Map<String, Object> processSheet(Sheet sheet, boolean overwrite) {
        ...
        List<CarrierData> validCarriers = new ArrayList<>();
        ...
        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            ...
            try {
                CarrierData data = parseRow(row, columnMap, i + 1);
        ...
        return executeImport(validCarriers, overwrite, errors);
    }
    ...
    private CarrierData parseRow(Row row, Map<String, Integer> columnMap, int rowNum) {
    ...
    @Transactional(rollbackFor = Exception.class)
    protected Map<String, Object> executeImport(List<CarrierData> carriers, boolean overwrite, List<String> errors) {
        log.info("执行运营商导入: 总数={}, 覆盖={}", carriers.size(), overwrite);

        int success = 0;
        int skipped = 0;

        List<String> skus = new ArrayList<>();
        for (CarrierData c : carriers) {
            skus.add(c.sku);
        }

        Map<String, CarrierLibrary> existingMap = new HashMap<>();
        List<CarrierLibrary> existing = carrierLibraryMapper.selectList(
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<CarrierLibrary>()
                        .in(CarrierLibrary::getSku, skus)
        );
        for (CarrierLibrary c : existing) {
            existingMap.put(c.getSku(), c);
        }

        for (CarrierData data : carriers) {
            try {
                CarrierLibrary existingCarrier = existingMap.get(data.sku);

                if (existingCarrier != null) {
                    if (overwrite) {
                        updateCarrierFromData(existingCarrier, data);
                        carrierLibraryMapper.updateById(existingCarrier);
                        success++;
                        log.debug("更新运营商: sku={}", data.sku);
                    } else {
                        skipped++;
                        errors.add(String.format("SKU %s: 已存在，已跳过", data.sku));
                    }
                } else {
                    CarrierLibrary newCarrier = createCarrierFromData(data);
                    carrierLibraryMapper.insert(newCarrier);
                    success++;
                    log.debug("新增运营商: sku={}", data.sku);
                }
            } catch (Exception e) {
                errors.add(String.format("SKU %s: %s", data.sku, e.getMessage()));
                log.error("导入运营商失败: sku={}, error={}", data.sku, e.getMessage());
            }
        }
        ...
        log.info("运营商导入完成: 成功={}, 跳过={}, 失败={}", success, skipped, errors.size());
        return result;
    }

    private CarrierLibrary createCarrierFromData(CarrierData data) {
        CarrierLibrary carrier = new CarrierLibrary();
        carrier.setSku(data.sku);
        carrier.setBatch(StringUtils.hasText(data.batch) ? data.batch : "");
        carrier.setDeveloper(StringUtils.hasText(data.developer) ? data.developer : "");
        carrier.setCarrier(StringUtils.hasText(data.carrier) ? data.carrier : "");
        carrier.setElement(StringUtils.hasText(data.element) ? data.element : "");
        carrier.setModificationRequirement(data.modificationRequirement);
        carrier.setInfringementLabel(data.infringementLabel);
        carrier.setImages(data.images);
        carrier.setReferenceImages(data.referenceImages);
        carrier.setStatus(StringUtils.hasText(data.status) ? data.status : "pending");
        carrier.setCreateTime(LocalDateTime.now());
        carrier.setUpdateTime(LocalDateTime.now());
        return carrier;
    }

    private void updateCarrierFromData(CarrierLibrary carrier, CarrierData data) {
        if (StringUtils.hasText(data.batch)) carrier.setBatch(data.batch);
        if (StringUtils.hasText(data.developer)) carrier.setDeveloper(data.developer);
        if (StringUtils.hasText(data.carrier)) carrier.setCarrier(data.carrier);
        if (StringUtils.hasText(data.element)) carrier.setElement(data.element);
        if (data.modificationRequirement != null) carrier.setModificationRequirement(data.modificationRequirement);
        if (data.infringementLabel != null) carrier.setInfringementLabel(data.infringementLabel);
        if (data.images != null) carrier.setImages(data.images);
        if (data.referenceImages != null) carrier.setReferenceImages(data.referenceImages);
        if (StringUtils.hasText(data.status)) carrier.setStatus(data.status);
        carrier.setUpdateTime(LocalDateTime.now());
    }
    ...
    public byte[] generateImportTemplate() {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            Sheet sheet = workbook.createSheet("运营商导入模板");
