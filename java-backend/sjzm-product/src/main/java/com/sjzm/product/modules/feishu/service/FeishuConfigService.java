...
/**
 * 飞书凭证配置服务。
 * AppId/AppSecret 优先读 api_config 表，缺失时回退 FeishuConfig（环境变量）。
 * 仿 LingxingConfigService 的 DB 覆盖 env 模式。
 */
@Slf4j
@Service
...
public class FeishuConfigService {
    ...
    private static final String KEY_APP_ID = "feishu_app_id";
    private static final String KEY_APP_SECRET = "feishu_app_secret";
    ...
    public void updateCredentials(String appId, String appSecret) {
        if (appId != null && !appId.isBlank()) saveConfig(KEY_APP_ID, appId, "飞书开放平台 AppId");
        if (appSecret != null && !appSecret.isBlank()) saveConfig(KEY_APP_SECRET, appSecret, "飞书开放平台 AppSecret");
        log.info("飞书凭证已更新");
    }
