...
| `sku_identifier` | SKU 识别码 | 否 | string | |
...
| `unit` | 单位（套、个、台） | 否 | string | |
| `category_id` | 分类 ID（与分类同时存在时，优先取分类 ID） | 否 | int | |
| `category` | 分类 | 否 | string | |
| `model` | 型号 | 否 | string | |
| `brand_id` | 品牌 ID（与品牌同时存在时，优先取品牌 ID） | 否 | int | |
| `brand` | 品牌 | 否 | string | |
| `open_status` | 开启状态：0-停用，1-启用 | 否 | int | |
| `status` | 状态（默认 1）：0-停售，1-在售，2-开发中，3-清仓 | 否 | int | |
...
| 参数名 | 说明 | 必填 | 类型 |
|--------|------|:----:|:----:|
| `cg_opt_uid` | 采购：采购员 ID（与采购员名同时填写时，以采购员 ID 为准） | 否 | int |
| `cg_opt_username` | 采购：采购员名 | 否 | string |
| `product_developer_uid` | 开发者 ID（与开发者名称同时填写时，以开发者 ID 为准） | 否 | int |
| `product_developer` | 开发者名称 | 否 | string |
| `product_creator_uid` | 创建人 ID，默认 API 账号 ID | 否 | int |
| `product_duty_uids` | 负责人 ID | 否 | array |
| `is_append_product_duty` | 负责人是否追加创建人：0-否，1-是；默认 1，只有编辑 SKU 时才生效 | 否 | int |
...
| `qc_standard>>custom_qc_template>>qc_image>>file_id` | 质检图片文件 ID | 否 | string |
| `qc_standard>>custom_qc_template>>qc_image>>customer_url` | 客户的质检图片 URL | 否 | string |

### 报关清关费用信息

支持国家同顶部列表。XX 为国家简码，如 US。

| 参数名 | 说明 | 必填 | 类型 |
|--------|------|:----:|:----:|
| `product_logistics_list` | 报关清关费用信息 | 否 | object |
| `product_logistics_list>>XX_cg_transport_costs` | 默认头程费用（含税） | 否 | string |
| `product_logistics_list>>XX_currency` | 默认头程费用币种 | 否 | string |
| `product_logistics_list>>XX_clearance_price` | 清关价格 | 否 | string |
| `product_logistics_list>>XX_clearance_price_currency` | 清关价格币种，默认 CNY | 否 | string |
| `product_logistics_list>>XX_bg_import_hs_code` | HS Code | 否 | string |
| `product_logistics_list>>XX_bg_tax_rate` | 税率 | 否 | string |

### 供应商报价信息

| 参数名 | 说明 | 必填 | 类型 |
|--------|------|:----:|:----:|
| `supplier_quote` | 供应商报价信息（传空值则清空产品供应商报价） | 否 | array |
| `supplier_quote>>erp_supplier_id` | 领星 ERP 供应商 ID（与 supplier_id 必填其一） | 是 | int |
| `supplier_quote>>supplier_id` | 客户系统供应商 ID（与 erp_supplier_id 必填其一） | 否 | int |
| `supplier_quote>>supplier_product_url` | 采购链接，最多 20 个，没有则传空数组 | 否 | array |
...
| `supplier_quote>>quotes>>currency` | 报价币种，目前只有 CNY 和 USD | 是 | string |
| `supplier_quote>>quotes>>is_tax` | 是否含税：0-否，1-是 | 是 | int |
| `supplier_quote>>quotes>>tax_rate` | 税率，为空则表示 0 | 是 | string |
| `supplier_quote>>quotes>>step_prices` | 阶梯价信息 | 是 | array |
| `supplier_quote>>quotes>>step_prices>>moq` | 最小起订量，最小值为 1 | 是 | int |
| `supplier_quote>>quotes>>step_prices>>price_with_tax` | 含税单价，4 位小数 | 是 | string |

### 产品特殊属性

| 参数名 | 说明 | 必填 | 类型 |
|--------|------|:----:|:----:|
| `special_attr` | 产品特殊属性：1-含电，2-纯电，3-液体，4-粉末，5-膏体，6-带磁，7-纺织品，8-普货（普货与其他选项互斥） | 否 | array |
...
| `declaration>>customs_declaration_hs_code` | 报关：报关 HSCODE | 否 | string |
| `declaration>>other_declare_element` | 报关：其他申报要素 | 否 | string |
| `declaration>>customs_declaration_exempt` | 报关：征免 | 否 | string |
| `declaration>>production_and_sales_enterprise_name` | 报关：生产销售企业名称 | 否 | string |
| `declaration>>production_and_sales_enterprise_code` | 报关：生产销售企业代码 | 否 | string |
...
| `clearance>>customs_clearance_preferential` | 清关：出口享惠情况：1-不享惠，2-享惠，3-不确定 | 否 | int |
| `clearance>>customs_clearance_brand_type` | 清关：品牌类型：1-无品牌，2-境内自主，3-境内收购，4-境外（贴牌），5-境外（其他） | 否 | int |
| `clearance>>customs_clearance_product_pattern` | 清关：产品型号 | 否 | string |
| `clearance>>allocation_remark` | 清关：配货备注 | 否 | string |
| `clearance>>weaving_mode` | 织造方式：1-针织，2-梭织 | 否 | int |
...
| `aux_relation_list>>aux_sku` | 辅料 SKU | 是 | string |
...
| 参数名 | 说明 | 必填 | 类型 |
|--------|------|:----:|:----:|
| `api_spu` | SPU（填写时属性列表必填，同时填写且为空时取消关联） | 否 | string |
| `api_spu_attribute` | 属性列表（填写时 SPU 必填，同时填写且为空时取消关联） | 否 | array |
| `api_spu_attribute>>pa_id` | 属性 ID | 是 | int |
| `api_spu_attribute>>pai_id` | 属性值 ID | 是 | int |

### 自定义字段

| 参数名 | 说明 | 必填 | 类型 |
|--------|------|:----:|:----:|
| `custom_fields` | 自定义字段 | 否 | array |
| `custom_fields>>id` | 自定义字段 ID | 否 | string |
| `custom_fields>>val` | 自定义字段值 | 是 | string |
| `custom_fields>>character` | 自定义字段值的"属性/单位/币种" | 否 | string |
...
| `request_id` | 请求链路 ID | 是 | string | |
| `response_time` | 响应时间 | 是 | string | 2020-04-30 17:33:32 |
| `data` | 响应数据 | 是 | object | |
| `data>>product_id` | 本地产品 ID | 是 | int | 1010 |
| `data>>sku` | 本地产品 SKU | 是 | string | SKU2986DFA1 |
| `data>>sku_identifier` | SKU 识别码 | 是 | string | SKU2986DFA1 |
