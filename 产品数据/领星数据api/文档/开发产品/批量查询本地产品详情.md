...
| 参数名 | 说明 | 必填 | 类型 | 示例 |
|--------|------|:----:|:----:|------|
| `productIds` | 产品 ID，上限 100 个（产品 ID、产品 SKU、SKU 识别码 三选一必填） | 否 | array | `["10001","10002"]` |
| `skus` | 产品 SKU，上限 100 个（产品 ID、产品 SKU、SKU 识别码 三选一必填） | 否 | array | `["ceshi001","测试002"]` |
| `sku_identifiers` | SKU 识别码，上限 100 个（产品 ID、产品 SKU、SKU 识别码 三选一必填） | 否 | array | `["ceshi001","测试002"]` |
...
| `data` | 响应数据 | 是 | array | |
| `data>>id` | 本地产品 ID | 是 | int | |
| `data>>product_name` | 产品名称 | 是 | string | 00.100 |
| `data>>sku` | 产品 SKU | 是 | string | |
...
| `data>>cid` | 分类 ID | 是 | int | |
| `data>>bid` | 品牌 ID | 是 | int | |
...
| `data>>is_combo` | 是否组合产品：0-否，1-是 | 是 | int | |
...
| `data>>currency` | 中国官方汇率 code | 是 | string | |
...
| `data>>cg_product_length` | 采购：产品规格-长（CM） | 是 | number | |
| `data>>cg_product_width` | 采购：产品规格-宽（CM） | 是 | number | |
| `data>>cg_product_height` | 采购：产品规格-高（CM） | 是 | number | |
| `data>>cg_package_length` | 采购：包装规格-长（CM） | 是 | number | |
| `data>>cg_package_width` | 采购：包装规格-宽（CM） | 是 | number | |
| `data>>cg_package_height` | 采购：包装规格-高（CM） | 是 | number | |
| `data>>cg_box_length` | 采购：外箱规格-长（CM） | 是 | number | |
| `data>>cg_box_width` | 采购：外箱规格-宽（CM） | 是 | number | |
| `data>>cg_box_height` | 采购：外箱规格-高（CM） | 是 | number | |
...
| `data>>bg_import_hs_code` | 【已废弃字段】报关：HS Code（进口国） | 是 | string | |
| `data>>bg_tax_rate` | 【已废弃字段】报关：税率 | 是 | number | |
...
| `data>>qc_standard>>custom_qc_template>>qc_image>>file_id` | 领星文件 ID | 是 | int | |
| `data>>qc_standard>>custom_qc_template>>qc_image>>customer_url` | 客户的图片 URL | 是 | string | |
...
| `data>>product_logistics_relation` | 物料关联【XX 为国家简码，比如美国 US】 | 是 | array | |
