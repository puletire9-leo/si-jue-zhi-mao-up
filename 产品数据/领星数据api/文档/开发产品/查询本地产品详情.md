...
**product_logistics_relation 参数说明**：数组子元素中各前缀的含义对应国家简码：
...
| 参数名 | 说明 | 必填 | 类型 | 示例 |
|--------|------|:----:|:----:|------|
| `id` | 产品 ID（产品 ID、产品 SKU、SKU 识别码 三选一必填） | 否 | int | 10001 |
| `sku` | 产品 SKU（产品 ID、产品 SKU、SKU 识别码 三选一必填） | 否 | string | ceshi001 |
| `sku_identifier` | SKU 识别码（产品 ID、产品 SKU、SKU 识别码 三选一必填） | 否 | string | ceshi001 |
...
| `request_id` | 请求链路 ID | 是 | string | F44E6A41-5219-4FEF-DD46-5BCC4B8305E1 |
| `response_time` | 响应时间 | 是 | string | 2021-03-19 11:53:49 |
| `data` | 产品数据 | 是 | object | |
| `data>>id` | 本地产品 ID | 是 | int | 1 |
| `data>>product_name` | 产品名称 | 是 | string | 00.100 |
| `data>>sku` | 产品 SKU | 是 | string | 视觉111 |
| `data>>sku_identifier` | SKU 识别码 | 是 | string | 视觉111 |
...
| `data>>attachment_id` | 附件 ID | 是 | array | [123] |
| `data>>special_attr` | 产品特殊属性：1-含电，2-纯电，3-液体，4-粉末，5-膏体，6-带磁 | 是 | array | [1,2,3] |
| `data>>currency` | 中国官方汇率 code | 是 | string | USD |
...
| `data>>cg_product_length` | 采购：产品规格-长（CM） | 是 | number | 12.0 |
| `data>>cg_product_width` | 采购：产品规格-宽（CM） | 是 | number | 13.0 |
| `data>>cg_product_height` | 采购：产品规格-高（CM） | 是 | number | 14.0 |
| `data>>cg_package_length` | 采购：包装规格-长（CM） | 是 | number | 16.0 |
| `data>>cg_package_width` | 采购：包装规格-宽（CM） | 是 | number | 17.0 |
| `data>>cg_package_height` | 采购：包装规格-高（CM） | 是 | number | 18.0 |
| `data>>cg_box_length` | 采购：外箱规格-长（CM） | 是 | number | 20.0 |
| `data>>cg_box_width` | 采购：外箱规格-宽（CM） | 是 | number | 21.0 |
| `data>>cg_box_height` | 采购：外箱规格-高（CM） | 是 | number | 22.0 |
...
| `data>>custom_fields>>id` | 字段 ID | 是 | string | |
...
| `data>>bg_export_hs_code` | 报关：HS Code（出口国）【中国 HS Code】 | 是 | string | |
| `data>>bg_import_hs_code` | 报关：HS Code（进口国）【美国 HS Code】 | 是 | string | |
...
| `data>>qc_standard>>custom_qc_template>>qc_image` | 质检图片（字段待废弃，请使用 qc_images） | 是 | array | |
| `data>>qc_standard>>custom_qc_template>>qc_image>>file_id` | 领星文件 ID | 否 | int | |
| `data>>qc_standard>>custom_qc_template>>qc_image>>customer_url` | 客户的图片 URL | 否 | string | |
| `data>>qc_standard>>custom_qc_template>>qc_images` | 质检图片-新 | 是 | array | |
| `data>>qc_standard>>custom_qc_template>>qc_images>>file_id` | 领星文件 ID | 是 | string | |
| `data>>supplier_quote` | 供应商报价数据 | 是 | array | |
| `data>>supplier_quote>>psq_id` | 供应商报价 ID | 是 | string | |
| `data>>supplier_quote>>product_id` | 产品 ID | 是 | int | |
| `data>>supplier_quote>>supplier_id` | 供应商 ID | 是 | int | |
...
| `data>>combo_product_list>>product_id` | 本地产品 ID | 是 | int | |
| `data>>combo_product_list>>quantity` | 数量 | 是 | int | |
| `data>>combo_product_list>>sku` | SKU | 是 | string | |
| `data>>product_logistics_relation` | 物流关联【XX 为国家简码，比如美国 US】 | 是 | array | |
| `data>>product_logistics_relation>>XX_cg_transport_costs` | 默认头程成本（含税） | 是 | number | |
| `data>>product_logistics_relation>>XX_currency` | 官方汇率 code | 是 | string | |
...
| `data>>product_logistics_relation>>XX_bg_import_hs_code` | 报关：HS Code（进口国） | 是 | string | |
...
| `data>>declaration>>production_and_sales_enterprise_name` | 报关：生产销售企业名称 | 是 | string | |
| `data>>declaration>>production_and_sales_enterprise_code` | 报关：生产销售企业代码 | 是 | string | |
...
| `data>>clearance>>customs_clearance_preferential` | 清关：出口享惠情况：1-不享惠，2-享惠，3-不确定 | 是 | int | |
| `data>>clearance>>customs_clearance_brand_type` | 清关：品牌类型：1-无品牌，2-境内自主，3-境内收购，4-境外（贴牌），5-境外（其他） | 是 | int | |
...
| `data>>clearance>>customs_clearance_hs_code` | 默认清关 HS CODE | 是 | string | |
...
| `data>>aux_relation_list>>aux_sku` | 辅料 SKU | 是 | string | |
...
| `data>>category_full_name` | 完整分类层级名称 | 是 | string | |
        ...
        "supplier_quote": [
            {
                ...
                "supplier_id": "317",
                "is_primary": 1,
                "supplier_product_url": ["https://xxx/xxx"],
