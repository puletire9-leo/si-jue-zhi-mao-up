...
# Plugin web providers removed — web tools unavailable in selection-agent.
# Stub re-exports for backward-compat so the module still loads.
Firecrawl = None  # type: ignore
_firecrawl_backend_help_suffix = ""
def _get_firecrawl_client(*a, **kw): return None
def _get_firecrawl_gateway_url(*a, **kw): return None
def _is_tool_gateway_ready(*a, **kw): return False
def check_firecrawl_api_key(*a, **kw): return False
def _normalize_tavily_documents(*a, **kw): return []
def _normalize_tavily_search_results(*a, **kw): return []
def _tavily_request(*a, **kw): return None
def _get_parallel_client(*a, **kw): return None
def _get_async_parallel_client(*a, **kw): return None
def _get_exa_client(*a, **kw): return None
...
try:
    ...
    async_call_llm = None
    def extract_content_or_reasoning(*a, **kw): return None
    def get_async_text_auxiliary_client(*a, **kw): return None, None
