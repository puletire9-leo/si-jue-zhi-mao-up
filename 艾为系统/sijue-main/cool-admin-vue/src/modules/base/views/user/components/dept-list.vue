...
/** 左侧树「未分配」虚拟节点 id（非真实部门） */
const UNASSIGNED_DEPT_ID = 0;

// 树形列表
const list = ref<Eps.BaseSysDepartmentEntity[]>([]);

function isUnassignedNode(item?: { id?: number; _unassigned?: boolean }) {
	return !!item?._unassigned || item?.id === UNASSIGNED_DEPT_ID;
}

/** 在组织架构根节点下插入「未分配」 */
function injectUnassignedNode(tree: Eps.BaseSysDepartmentEntity[]) {
	if (!tree.length) return;
	const root = tree[0];
	if (!root.children) {
		root.children = [];
	}
	if (root.children.some((c: any) => c._unassigned)) return;
...
function allowDrag({ data }: any) {
	return data.parentId && !data._unassigned;
}
	...
	if (item) {
		...
		nextTick(() => {
			if (isUnassignedNode(item)) {
				emit("refresh", { page: 1, onlyUnassigned: true });
				return;
			}
			...
			const ids = children.length ? revDeepTree(children).map((e) => e.id) : [];
			ids.unshift(item.id);
...
function onContextMenu(e: any, d?: any, n?: any) {
	...
	const isUnassigned = isUnassignedNode(d);
	...
	ContextMenu.open(e, {
		list: [
			...
			{
				label: "编辑",
				hidden: isUnassigned || !checkPerm(perm.update),
				callback(done) {
					rowEdit(d);
					done();
				}
			...
			{
				label: "删除",
				hidden: isUnassigned || !d.parentId || !checkPerm(perm.delete),
				callback(done) {
					rowDel(d);
					done();
				}
