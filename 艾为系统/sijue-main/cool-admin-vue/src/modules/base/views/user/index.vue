		...
		<template #right>
			<cl-crud ref="Crud">
				<cl-row>
					...
					<el-button type="success" @click="synLingxingID()"> 同步零星ID </el-button>
					<cl-flex1 />
...
const Upsert = useUpsert({
	...
	onSubmit(data, { next }) {
		const sel = ViewGroup.value?.selected as {
			id?: number;
			_unassigned?: boolean;
		};
		const departmentId =
			sel?._unassigned || sel?.id === 0 ? null : sel?.id;
		next({
			departmentId,
			...data
		});
...
// 新增成员
function onUserAdd(
	item: Eps.BaseSysDepartmentEntity & { _unassigned?: boolean }
) {
	const departmentId =
		item._unassigned || item.id === 0 ? undefined : item.id;
