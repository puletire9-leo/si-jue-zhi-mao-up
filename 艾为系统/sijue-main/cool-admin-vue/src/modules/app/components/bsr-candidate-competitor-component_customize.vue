...
const fetchCountryStatus = async () => {
	const res = await service.app.bsr_candidate_competitor_customize.page({
		candidate_id: props.candidate?.asinid,
		status: [1, 2],
		size: 1000
	});
...
const handleStatusChange = async (row: any, newStatus: string) => {
	try {
		...
		await service.app.bsr_candidate_competitor_customize.update(updateData);
...
const batchSubmit = async () => {
	...
	try {
		...
		await service.app.bsr_candidate_competitor_customize.add(payload);
...
const handleRemoveDuplicates = async () => {
	try {
		...
		try {
			...
			await service.app.bsr_candidate_competitor_customize.delete({ ids: idsToDelete });
