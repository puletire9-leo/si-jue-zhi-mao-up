<template>
		...
		<div class="dashboard-split-wrapper" ref="splitWrapperRef">
			...
			>
				...
				<div
					...
					:style="{ height: verticalUpperHeightPx + 'px', minHeight: '300px' }"
				>
						...
						<div class="chart-header">
							...
							<div class="keyword-tags">
								<el-tooltip
									...
									:key="kw.name"
									:content="kw.isDefault ? '默认关键词（用户手动设置）' : '自动补充（按流量得分排序）'"
									placement="top"
								>
									<el-check-tag
										:checked="kw.selected"
										...
										class="kw-tag"
										:class="{ 'kw-tag-default': kw.isDefault }"
										:style="kw.isDefault ? { '--el-color-primary': '#D97706', '--el-color-primary-light-3': '#f59e0b', '--el-color-primary-light-5': '#fbbf24', '--el-color-primary-light-7': '#fcd34d', '--el-color-primary-light-8': '#fde68a', '--el-color-primary-light-9': '#fef3c7', '--el-fill-color-light': '#FFF7ED' } : {}"
									>
										<span v-if="kw.isDefault" style="margin-right: 2px; font-size: 10px;">⭐</span>
										{{ kw.name }}
									</el-check-tag>
								</el-tooltip>
							</div>
						...
						>
							<div
								...
								:class="{
									...
									'has-custom-alpha': idx === 3 && customAlpha !== undefined
								}"
								@click="handleAlgoClick(idx)"
							>
							<!-- 综合走势卡片 -->
							<div v-if="idx === 3" class="card-inner">
								<span class="m-label">
									{{ algo }}
									<el-tooltip v-if="algoSelection === 4" placement="top" :show-after="200">
										...
										<span class="alpha-gear" @click.stop="alphaDialogVisible = true">⚙</span>
									</el-tooltip>
								</span>
	...
	<!-- α 权重配置弹窗 -->
	<AlphaConfigDialog
		v-model:visible="alphaDialogVisible"
		:listing="listing"
		:calendarData="calendarCoefficients"
		:dailyAvgSales="baseDailyAvgSales"
		:selectionStart="selectionStart"
		:selectionEnd="selectionEnd"
		:customAlpha="customAlpha"
		:customMonthlyAlphas="customMonthlyAlphas"
		@apply="onAlphaApply"
		@save="onAlphaSave"
		@reset="onAlphaReset"
	/>
...
import AlphaConfigDialog from "./AlphaConfigDialog.vue";
...
const verticalUpperHeightPx = ref(400); // 初始像素高度 (会在 onMounted 更新)
...
onMounted(async () => {
	...
	nextTick(() => {
		if (keywordsColumnRef.value) {
			...
			// 初始比例改为动态：如果没有足够高度，按 50% 分配，绝不给死数字
			verticalUpperHeightPx.value = totalHeight > 100 ? Math.floor(totalHeight * 0.5) : 300;
		}
	...
	try {
		...
		if (latestRecord && latestRecord.expected_sales) {
			...
			expectedDemandInput.value = salesData.totalQty || undefined;
			...
			customMonthlyAlphas.value = salesData.customMonthlyAlphas || salesData.custom_monthly_alphas || {};
...
// --- 补货增强功能状态 ---
const expectedDemandInput = ref<number | undefined>(undefined); // 手动编辑的预计需求
const algoSelection = ref<number | null>(1); // 当前选中的算法 (1=日均单量, 2=历史销售, 3=搜索词趋势, 4=综合走势)，默认选中第一个
const algoNames = ["日均单量", "历史销售", "搜索词趋势", "综合走势"]; // 算法名称列表
...
// --- α 权重设置（综合走势算法）---
const customAlpha = ref<number | undefined>(undefined); // 自定义 α 权重，undefined = 走后端默认逻辑
const customMonthlyAlphas = ref<Record<string, number>>({}); // 逐月 α 覆盖
const alphaPopoverVisible = ref(false); // α 设置 Popover 可见性（保留兼容）
const alphaDialogVisible = ref(false); // α 配置弹窗可见性
const alphaInputValue = ref(0.7); // α 滑块/输入框绑定值
...
interface ReplenishmentSegment {
	...
	dailyNeed: number;
	subtotal: number;
	algoUsed: number; // 实际使用的算法 (1/2/3/4)，可能因数据不足降级
...
const algoAvailability = computed(() => {
	...
	const calendarData = calendarCoefficients.value;
	const algo4Available =
		calendarData &&
		Object.keys(calendarData).length > 0 &&
		Object.values(calendarData).some((d: any) => d.combined?.coefficient !== undefined);

	return [algo1Available, algo2Available, algo3Available, algo4Available];
});

// 前向声明占位符 - 实际函数在后面定义
const _lazyAlgoForecasts = ref<number[]>([0, 0, 0, 0]);
...
const handleAlgoClick = (index: number) => {
	...
	// 综合走势已激活时，再次点击打开 α 配置弹窗
	if (index === 3 && algoSelection.value === 4) {
		alphaDialogVisible.value = true;
		return;
	}
	// 切换到其他算法时关闭 α 设置面板
...
// α 配置弹窗：应用回调（手动调整模式）
const onAlphaApply = (payload: { mode: string; alpha: number; monthlyAlphas: Record<string, number> }) => {
	customAlpha.value = payload.alpha;
	customMonthlyAlphas.value = { ...payload.monthlyAlphas };
	alphaInputValue.value = payload.alpha;
	console.log('[α配置] 应用:', payload);
};

// α 配置弹窗：保存回调（用户配置模式）— 保存后清手动，重拉日历数据
const onAlphaSave = async (payload: { mode: string; alpha: number; monthlyAlphas: Record<string, number> }) => {
	// 保存用户配置后，清掉手动调整（用户配置覆盖手动）
	customAlpha.value = undefined;
	customMonthlyAlphas.value = {};
	alphaInputValue.value = payload.alpha;
	console.log('[α配置] 保存，清除手动调整:', payload);
	// 重拉日历数据，让后端用新的用户配置重新计算 combined.coefficient
	await loadCalendarData();
};

// α 配置弹窗：系统自动模式关闭 → 清除旧的 customAlpha 覆盖
const onAlphaReset = () => {
	customAlpha.value = undefined;
	customMonthlyAlphas.value = {};
	alphaInputValue.value = 0.7;
	console.log('[α配置] 重置为系统自动');
};
...
const selectAlgo = async (index: number) => {
	...
	if (algo === 2) {
		...
		} else if (!Object.values(calendarData).some((d: any) => d.combined?.coefficient !== undefined)) {
...
const generatedRemark = computed(() => {
	...
	const remarkData = {
		...
		custom_alpha: customAlpha.value, // 自定义 α 权重（undefined 表示走默认）
		custom_monthly_alphas: Object.keys(customMonthlyAlphas.value).length > 0 ? customMonthlyAlphas.value : undefined,
	...
	try {
		...
		const result = await service.app.bsr_analysis_record_lingxing.save({
			...
			expected_sales: {
				...
				customAlpha: remarkObj.custom_alpha // 自定义 α 权重
			},
		...
		expectedDemandInput.value = undefined;
		manualRemark.value = "";
	...
	try {
		...
		if (res) {
			...
			if (res.series && Array.isArray(res.series)) {
				// 过滤掉在图表实际展示范围（后12个月）内搜索量全为0的关键词
				// 因为图表只展示后12个月（startIndex=1），第1个月仅作基数，不画在图上
				const displayStartIdx = Math.max(0, (res.xAxis?.length || 13) - 12);
				const validSeries = res.series.filter((item: any) => {
					const displayData = (item.data || []).slice(displayStartIdx);
					return displayData.some((val: number) => val > 0);
				});

				allKeywords.value = validSeries.map((item: any, index: number) => ({
					name: item.name,
					selected: true, // 默认选中
...
// ================== 日历模式系数数据 (新接口) ==================
// 存储后端预计算的系数数据，格式: { '2025-12': { sales: {...}, keywords: {...}, combined: {...} }, ... }
const calendarCoefficients = ref<
	Record<
		string,
		{
			...
			combined?: { coefficient: number; alpha: number; sales_filled: boolean; filled_sales_coefficient: number; keyword_coefficient: number };
		}
	>
>({});
// 基准月信息
	...
	if (calendarData) {
		if (algorithm === 2) {
			...
			if (combinedData && combinedData.coefficient !== undefined) {
				let coeff = combinedData.coefficient; // 默认用后端预计算值
				const sc = combinedData.filled_sales_coefficient;
				const kc = combinedData.keyword_coefficient;
				if (sc !== undefined && kc !== undefined) {
					// 优先查逐月α → 全局α → 后端值
					const monthAlpha = customMonthlyAlphas.value[targetMonthStr] ?? customAlpha.value;
					if (monthAlpha !== undefined) {
						coeff = monthAlpha * sc + (1 - monthAlpha) * kc;
					}
				}
				return {
					valid: true,
...
interface CoefficientWithFallback {
	coefficient: number;
	algoUsed: number; // 实际使用的算法 (1=日均, 2=历史, 3=搜索词, 4=综合走势)
...
const getMonthCoefficientWithFallback = (
	...
	if (result.valid) {
		...
		const coeff = result.coefficient !== undefined
			? result.coefficient
			: (result.base > 0 ? result.target / result.base : 1);
...
// 监听选择变化，更新补货计算结果
watch(
	[selectionStart, selectionEnd, algoSelection, customAlpha, customMonthlyAlphas],
	() => {
		// 更新当前选中算法的结果
		if (selectionStart.value && selectionEnd.value) {
			...
			// 更新三种算法的预测值
			_lazyAlgoForecasts.value = [1, 2, 3, 4].map((algo) => {
			...
			_lazyAlgoForecasts.value = [0, 0, 0, 0];
		}
	},
	{ immediate: true }
);
...
watch(
	[algoSelection, customAlpha, customMonthlyAlphas],
	() => {
	...
	(newVal) => {
		if (newVal && Object.keys(newVal).length > 0) {
			...
			if (selectionStart.value && selectionEnd.value) {
				...
				_lazyAlgoForecasts.value = [1, 2, 3, 4].map((a) => {
...
const allKeywords = ref<{ name: string; selected: boolean; color: string; data: number[]; total?: number; isDefault: boolean }[]>([]);
...
.kw-tag-default {
	--el-color-primary: #D97706 !important;
	--el-color-primary-light-3: #f59e0b !important;
	--el-color-primary-light-5: #fbbf24 !important;
	--el-color-primary-light-7: #fcd34d !important;
	--el-color-primary-light-8: #fde68a !important;
	--el-color-primary-light-9: #fef3c7 !important;
	--el-fill-color-light: #FFF7ED !important;
}
...
.calendar-scroll-container {
	...
	display: flex;
	flex-direction: column;
	overflow: hidden; /* 默认双列模式下禁用内部滚动，由外部拉动 Resizer 展示全部 */
	padding: 0;
	transition: all 0.3s ease;
}
...
/* 智能美化滚动条 (仅窄屏可见) */
.calendar-scroll-container.is-narrow-mode::-webkit-scrollbar {
	width: 5px;
}
.calendar-scroll-container.is-narrow-mode::-webkit-scrollbar-track {
	background: rgba(0, 0, 0, 0.02);
	border-radius: 10px;
}
.calendar-scroll-container.is-narrow-mode::-webkit-scrollbar-thumb {
...
.calendar-grid {
	...
	gap: 16px;
	padding: 8px 8px 80px 8px; /* 底部留白增加，确保最后一行不被底栏遮挡 */
	height: max-content;
...
.calendar-grid.is-narrow {
	...
	padding-bottom: 80px; /* 仅在滚动模式下预留给补货条空间 */
}
...
.days-grid {
	display: grid;
	grid-template-columns: repeat(7, 1fr);
	gap: 4px; /* 从 2px 增加到 4px，产生清晰的间隔 */
}

.day-cell {
	width: 100%;
	min-height: 42px; /* 增加到 42px，确保即使有图标也不截断 */
	display: flex;
	flex-direction: column;
	align-items: center;
	...
	position: relative;
	gap: 2px;
	overflow: visible; /* 改为 visible，配合 padding 防止意外截断 */
	border: 1px solid transparent;
	/* GPU 加速优化 */
...
/* 里程碑底部悬浮角标 (专业定制方案) - 不撑大格子，明显展示在底部 */
.milestone-badge {
	position: absolute;
	bottom: 0px;
	left: 50%;
...
.day-cell.is-selected .milestone-badge {
	background: #fff;
	color: #409eff;
}
</style>
