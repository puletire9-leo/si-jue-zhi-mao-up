<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<!-- <el-button type="primary" @click="handleExportSelectedCompetitors">
        导出勾选关键词
      </el-button> -->
			<el-button type="primary" @click="handleFetchKeywords"> 获取勾选关键词 </el-button>
			<el-button
				type="success"
				@click="handleBatchGenerateLocalSKU"
				:loading="batchGeneratingSKU"
			>
				生成勾选本地SKU数据
			</el-button>
		...
		<cl-upsert ref="Upsert">
			...
			<template #slot-factory_links="{ scope }">
				<div class="factory-links-section">
					...
					<el-table :data="scope.variant_Combination" border style="width: 100%">
						...
						<el-table-column label="工厂链接组合" width="500" align="center">
							<template #default="{ row }">
								<el-select
			...
			<template #slot-btns="{ scope }">
				<el-button
					type="info"
					@click="handleGenerateLocalSKU2(scope)"
			...
			<template #slot-purchaserNum="{ scope }">
				<div class="purchase-section">
					...
					>
						...
						<el-table-column label="采购意见" width="280" align="center">
							<template #default="{ row }">
								<!-- 输入框组件 -->
						...
						<!-- 合计列 -->
						<el-table-column label="合计" width="100" align="center" prop="groupTotal">
							<template #default="{ row }">
								{{ row.groupTotal }}
							</template>
						</el-table-column>
...
const Crud = useCrud(
	{
		...
		async onRefresh(params, { next, done, render }) {
			...
			// 获取当前已存在的 Table 数据
			const currentData = Table.value?.data || [];

			// 合并当前数据和新数据，并进行全局去重
			const mergedList = [
				...currentData, // 保留当前已有的数据
				...list // 将新获取的数据直接合并
			];

			// 使用 Set 去重，确保每条数据 id 唯一
			...
			// 更新 Table 数据
			Table.value?.data && (Table.value.data = uniqueList);
			preloadProductImages(uniqueList, { reason: "bsr-candidate5" });
		}
	},
	(app) => {
		app.refresh();
...
const Table = useTable({
	columns: [
		...
		{ label: "产品页", prop: "dp_url", fixed: "left", minWidth: 100 },
		{
			label: "产品标题",
			prop: "item_name",
		...
		{
			label: "状态",
			prop: "is_generate_status",
			dict: [
				{ label: "未导入", value: 2, type: "warning" },
				{ label: "已导入", value: 4, type: "success" }
			],
			width: 85
		},
		{
...
const Upsert = useUpsert({
	...
	async onSubmit(data, { next, done }) {
		try {
			...
			if (data.variant_Combination) {
				data.variant_Combination = data.variant_Combination
					.filter((v: any) => v.name && v.selectedGroups && v.selectedGroups.length > 0)
					.map((variant: any) => {
						const groupProportions = {};
						variant.selectedGroups.forEach((groupName: string) => {
							const value = variant.groupProportions[groupName];
							groupProportions[groupName] =
						...
						return {
							id: variant.id,
							name: variant.name,
							selectedGroups: variant.selectedGroups,
							groupProportions
						};
					});
			}
...
const batchGeneratingSKU = ref(false);

async function handleBatchGenerateLocalSKU() {
	const selectedRows = Table.value?.getSelectionRows();
	if (!selectedRows || selectedRows.length === 0) {
		ElMessage.warning("请先选择要生成SKU的候选产品");
		return;
	}

	batchGeneratingSKU.value = true;
	let successCount = 0;
	let failCount = 0;
	...
	for (const row of selectedRows) {
		try {
			...
			try {
				...
				ElMessage.error(`检查ASIN ${row.asin} 记录时失败`);
				throw error;
			}
			...
			const data = {
				...
				bsr_html: row.bsr_html,
				bsr_category: row.bsr_category,
				bsr_rank: row.bsr_rank,
				bsr_link: row.bsr_link,

				dispatches_from: row.dispatches_from,
				sold_by: row.sold_by,
				date_first_available: row.date_first_available,
				...
				first_leg_freight: row.first_leg_freight,
				fba_freight: row.fba_freight,
				tax_rate: row.tax_rate,
				exchange_rate: row.exchange_rate,

				status: row.status,
				competitor_spider_status: row.competitor_spider_status,
				competitor_spider_res: row.competitor_spider_res,
				competitor_spider_time: row.competitor_spider_time,

				describe: row.describe,
				factory_links: row.factory_links,
				keyword_screenshots: row.keyword_screenshots,
				patent_memo: row.patent_memo,
				remark: row.remark,
			...
			if (existingRecord) {
				...
				ElMessage.success(`ASIN ${row.asin} 已更新`);
			} else {
				// 不存在相同记录，执行添加操作
				await service.app.bsr_candidate_customize.add(data);
				ElMessage.success(`ASIN ${row.asin} 已添加`);
			}
			...
			// 	// 检查竞品是否已存在
			// 	const { list: existingCompetitors } =
			// 		await service.app.bsr_candidate_competitor_customize.page({
			...
			// 		remark: `自动生成自候选产品 ${row.asin}`
			// 	};

			// 	if (existingCompetitors.length > 0) {
			// 		const existingComp = existingCompetitors[0];
			// 		if (existingComp) {
			// 			await service.app.bsr_candidate_competitor_customize.update({
			// 				id: existingComp.id,
			// 				...competitorData
			// 			});
			// 		}
			// 	} else {
			// 		await service.app.bsr_candidate_competitor_customize.add(competitorData);
			// 	}
			// }
			...
			console.error(`处理ASIN ${row.asin} 时出错:`, error);
			ElMessage.error(
				`处理ASIN ${row.asin} 时失败: ${error instanceof Error ? error.message : "未知错误"}`
			);
			failCount++;
		}
	}

	batchGeneratingSKU.value = false;
	ElMessage.info(`批量生成SKU完成。成功: ${successCount}个，失败: ${failCount}个`);

	// 刷新表格
	Crud.value?.refresh();
}

async function handleGenerateLocalSKU2(row: any) {
	...
	try {
		...
		if (existingRecord) {
			// 存在相同记录，执行更新操作
			// TypeScript能确定此时existingRecord不为null，可安全访问id
			await service.app.bsr_candidate_customize.update({
				id: existingRecord.id,
				...data
			});
		...
		// 		dispatches_from: competitor.dispatches_from,
		// 		sold_by: competitor.sold_by,
		// 		dispatches_type: competitor.dispatches_type,

		// 		date_first_available: competitor.date_first_available,
		// 		Main_monthly_sales: competitor.Main_monthly_sales,
		// 		Main_monthly_sales_sub: competitor.Main_monthly_sales_sub,
		// 		stock_quantity: competitor.stock_quantity,
		...
		// 		// 其他字段
		// 		remark: `自动生成自候选产品 ${row.asin}`,
		// 		sales_volume_data: competitor.sales_volume_data,
		// 		bsr_node: competitor.bsr_node,
		// 		bsr_node_rank: competitor.bsr_node_rank,
		// 		bsr_node_id: competitor.bsr_node_id,
		// 		sold_byID: competitor.sold_byID
		// 	};
...
async function handleExportSelectedCompetitors() {
	try {
		// 获取表格选择的候选产品ID
		...
		// 调用导出接口
		const response = await service.app.bsr_candidate.exportSelectedCompetitors({
			ids: selectedIds
		});
		...
		ElMessage.success("关键词信息导出成功");
	} catch (error) {
		console.error("导出失败:", error);
		ElMessage.error("关键词信息导出失败");
	}
}
...
async function handleFetchKeywords() {
	try {
		...
		if (!selectedIds || selectedIds.length === 0) {
			ElMessage.warning("请先选择要获取关键词的候选产品");
			return;
		}
...
const handleEdit = async (row: any) => {
	try {
		...
		resetCalculator(sysName);
		console.log("sids:", userStore.info);
...
async function deletePurchaser(row) {
	...
	// 从本地数据中移除
	const index = Upsert.value.form.purchasers.findIndex(
		(p) => p.id === row.id || p.purchaser === row.purchaser
	);
...
const handleDecision = async (scope: any, status: number) => {
	try {
		...
		// 2. 查找当前用户的所有采购记录（包括重复记录）
		const currentUserRecords = scope.purchasers.filter((p: any) => p.purchaser === currentUser);
		...
		// 4. 调用后端接口
		await service.app.bsr_candidate.update(payload);
		ElMessage.success(
...
const isCurrentUserInvalid = (scope) => {
	const currentUser = userStore.info?.name;
	if (!currentUser) return false;

	// 查找当前用户的记录
	const userRecord = scope.purchasers.find((p) => p.purchaser === currentUser);

	// 如果找到记录且状态为3（不可做），返回true
	return userRecord?.is_generate === 3;
};
// 计算单行合计
...
async function onSubmit3(data: any, helpers: any = {}) {
	...
	try {
		...
		if (failedItems.length === 0) {
			...
			ElMessage.success(`导入${total} 条数据，正在计算评分，请稍后查看...`);
			done({ failedItems });
		}
	} catch (err: any) {
