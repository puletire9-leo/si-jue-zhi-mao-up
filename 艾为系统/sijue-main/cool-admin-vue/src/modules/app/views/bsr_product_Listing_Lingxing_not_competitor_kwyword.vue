...
<!-- 
			...
			<el-space direction="horizontal" style="margin-left: 10px;">
				<el-text>无竞品</el-text>
				<el-switch
					v-model="filters.noCompetitor"
					active-text="是"
					inactive-text="否"
					inline-prompt
					@change="handleFilterChange('noCompetitor', $event)"
				></el-switch>
				<el-divider direction="vertical"></el-divider>
				<el-text>无关键词</el-text>
				<el-switch
					v-model="filters.noKeyword"
					active-text="是"
					inactive-text="否"
					inline-prompt
					@change="handleFilterChange('noKeyword', $event)"
				></el-switch>
			</el-space>
						...
						<el-tooltip placement="top" effect="light">
							<template #content>
								<div v-if="scope.row.restocking?.fbaValidList?.length">
									<el-table
										:data="scope.row.restocking.fbaValidList"
										size="small"
										border
...
function getFbaInventoryQuantity(row: any) {
	...
	// 如果有 fbaValidList，优先根据 msku 过滤明细来汇总计算
	if (Array.isArray(list) && list.length > 0) {
		...
		if (matchedList.length > 0) {
			return matchedList.reduce((sum: number, item: any) => {
				const afnFulfillableQuantity = Number(item.afnFulfillableQuantity) || 0;
				const reservedFcTransfers = Number(item.reservedFcTransfers) || 0;
				const reservedFcProcessing = Number(item.reservedFcProcessing) || 0;
				return sum + afnFulfillableQuantity + reservedFcTransfers + reservedFcProcessing;
			}, 0);
		}
	}
	
	// Fallback 逻辑，使用后端同步好的行本身数据
	const afnFulfillableQuantity = Number(row?.afn_fulfillable_quantity) || 0;
	const reservedFcTransfers = Number(row?.reserved_fc_transfers) || 0;
	const reservedFcProcessing = Number(row?.reserved_fc_processing) || 0;
...
const Crud = useCrud(
	{
		...
		async onRefresh(params, { next }) {
			...
			// 根据无竞品和无关键词开关组合查询参数
			if (filters.value.noCompetitor && filters.value.noKeyword) {
				params.dataStatus = 3; // 无竞品且无关键词
			} else if (filters.value.noCompetitor) {
				params.dataStatus = 1; // 仅无竞品
			} else if (filters.value.noKeyword) {
				params.dataStatus = 2; // 仅无关键词
			} else {
				// 如果都不选，可以传一个能匹配正常的或什么都不匹配的状态，这里暂设为不传dataStatus，
				// 或根据后端逻辑处理。假设不传 dataStatus 就不过滤
				params.dataStatus = 4; // 用 4 表示两者都不选（如果后端没有处理4，则查询全部）
			}
			
			// 删除前端传递的辅助字段，避免后端报错
			delete params.noCompetitor;
			delete params.noKeyword;
			...
			preloadProductImages(rows, { reason: "bsr-product-listing-not-competitor-keyword" });
