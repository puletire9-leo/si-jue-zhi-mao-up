<template>
	<cl-crud ref="Crud">
		<cl-row>
			...
			<el-button type="success" @click="handleExport()"> 导出选品管理数据 </el-button>

			<el-button type="success" @click="synLingxingID()"> 同步领星ID </el-button>
		...
		<cl-upsert ref="Upsert">
						...
						<el-table-column label="操作" width="150" align="center">
							<template #default="{ row, $index }">
								...
								>
									<el-button
										type="text"
										:icon="row.locked ? 'Lock' : 'Unlock'"
										@click="toggleVariantLock(row)"
									>
										{{ row.locked ? "已锁定" : "可编辑" }}
									</el-button>
								</el-tooltip>
			...
			<template #slot-btns="{ scope }">
				<el-button type="warning" @click="handleGeneratePurchaseOrder(scope)">
					生成采购单
				</el-button>

				<el-button
					type="warning"
					@click="handleGenerateLocalSKU(scope)"
					:loading="generatingSKU"
				>
					生成领星SKU
				</el-button>

				<!-- <el-button type="info" @click="handleGenerateLocalSKU2(scope)" :loading="generatingSKU">
          生成本地SKU
        </el-button> -->
			...
			<template #slot-purchaserNum="{ scope }">
				<div class="purchase-section">
					...
					<div class="purchase-actions">
						<!-- <el-button type="success" @click="handleDecision(scope, 2)">
              做
            </el-button>
...
const Crud = useCrud(
	{
		...
		async onRefresh(params, { next, done, render }) {
			...
			preloadProductImages(rows, { reason: "bsr-candidate2" });
		}
	},
	(app) => {
		app.refresh();
...
const processDataByCountry = (list: any[], dataField: string) => {
	const countryMap: Record<string, number[]> = {
		英国: new Array(12).fill(0),
		德国: new Array(12).fill(0),
		法国: new Array(12).fill(0),
		西班牙: new Array(12).fill(0),
		意大利: new Array(12).fill(0)
	};

	list.forEach((item: any) => {
		const country = item.marketplace || item.marketplaces?.[0];
		if (!country || !countryMap[country]) return;

		try {
			const rawData =
				typeof item[dataField] === "string"
					? JSON.parse(item[dataField] || "[]")
					: item[dataField] || [];

			rawData.forEach(({ date, searches }: any) => {
				const month = parseInt(String(date).slice(-2)) - 1;
				if (month >= 0 && month < 12) {
					countryMap[country][month] += Number(searches) || 0;
				}
			});
		} catch (err) {
...
async function handleGenerateLocalSKU(scope: any) {
	try {
		generatingSKU.value = true;

		// 只保存factory_links和variant_Combination数据
		ensureCandidateIds(scope);
		...
		const selectedIndexes = scope.variant_Combination
			.map((variant: any, index: number) => (variant.selected ? index : -1))
		...
		if (skuResult?.lingxingSynced === false) {
			...
			ElMessage.success("领星SKU生成成功");
		}
		Upsert.value?.close();
	} catch (e) {
		console.error("生成SKU失败:", e);
		ElMessage.error("生成SKU失败: " + (e || "未知错误"));
	} finally {
...
async function handleGenerateLocalSKU2(row: any) {
	...
	try {
		try {
			...
			ElMessage.error("重复记录检查失败");
			throw error; // 重新抛出错误以进入外层catch块
		}
		await service.app.bsr_candidate_customize.add({
			// 基础信息
			asin: row.asin,
			...
			review_num: row.review_num,
			last_star: row.last_star,
			asinid: row.candidate_id,

			// BSR相关
			bsr_html: row.bsr_html,
			bsr_category: row.bsr_category, // 仅保留一个
			...
			// 特殊字段处理
			describe: row.describe, // JSON类型字段需序列化
...
// 在获取数据后初始化锁定状态
async function initLockStatus(data: any) {
	if (!data.factory_links || !data.variant_Combination) return;
	...
	data.variant_Combination.forEach((variant: any) => {
		// 确保locked属性存在
		if (variant.locked === undefined) {
			variant.locked = false;
		}

		// 更新链接锁定状态
		if (variant.locked) {
			variant.selectedLinks.forEach((linkName: string) => {
				const factoryLink = data.factory_links.find((link: any) => link.name === linkName);
				if (factoryLink) {
					factoryLink.locked = true;
				}
			});
		}
	});
...
function _getAnEmptyFactoryLinkItem() {
	return {
		...
		price: "",
		name: "",
		isNameManuallyModified: false // 新增字段，用于跟踪用户是否手动修改过品名
	};
}
...
async function addVariant(scope: any) {
	...
	scope.variant_Combination.push({
		...
		locked: false, // 初始化锁定状态
		uk_title: ukTitle,
		de_title: deTitle
	});
}

function toggleVariantLock(row) {
	row.locked = !row.locked;

	const factoryLinks = Upsert.value?.form.factory_links || [];
	row.selectedGroups.forEach((groupName) => {
		// 使用filter()获取所有匹配的链接
		const groupLinks = factoryLinks.filter((l) => l.name === groupName);
		groupLinks.forEach((link) => {
			link.locked = row.locked; // 锁定整个组
		});
	});
}
...
async function synLingxingID() {
	// 获取所有用户的 id 和 name
	const allUsers = await service.base.sys.user.list();
	const allUserData = allUsers.map((user: any) => ({ id: user.id, name: user.name })); // 获取所有用户的 id 和 name

	if (allUserData.length > 0) {
		// 将所有用户的 id 和 name 传递给后端
		const response = await service.app.bsr_candidate.synLingxingID({ users: allUserData });
		console.log(response); // 根据需求处理返回的数据
	} else {
		console.log("没有用户数据");
	}
}
...
async function handleExport() {
	try {
		// 发起请求
		const response = await service.app.bsr_candidate.exportCandidate2();
