<template>
	<div class="mini-analysis-container" v-loading="loading">
		<!-- 1. 聚合趋势图 -->
				...
				>
					...
					>
						<div class="card-inner">
							...
							<span v-if="selectionEnd && algoAvailability[idx]" class="m-val">{{
								algoForecasts[idx]
							}}</span>
							<span
...
const algoNames = ["日均单量", "历史销售", "搜索词趋势"];
...
onMounted(async () => {
	loadMiniData();
	loadCalendarData(); // 并行加载日历系数

	// 查询是否有暂存记录，如果有则回填数据（与 Charts 一致）
	try {
...
const algoForecasts = computed(() => {
	...
	return [calcTotal(1), calcTotal(2), calcTotal(3)];
});
...
.day-cell.past-day .day-icons-wrap {
	opacity: 0.6; /* Only dim the icons, not the whole cell/tooltip */
}
