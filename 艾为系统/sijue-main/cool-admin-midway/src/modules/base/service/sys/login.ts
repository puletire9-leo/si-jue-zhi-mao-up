...
import {AppAnyService} from "../../../app/service/any";
...
export class BaseSysLoginService extends BaseService {
  ...
  async login(login: LoginDTO) {
    ...
    if (checkV) {
      ...
      let sids = await this.appAnyService.getSidsByUserId(user.id);

      // 生成token
      const { expire, refreshExpire } = this.coolConfig.jwt.token;
      const result = {
        expire,
        token: await this.generateToken(user, roleIds, expire, sids),
        refreshExpire,
   ...
   * @param sids 角色授权的店铺 id 列表
   */
  async generateToken(user, roleIds, expire, sids, isRefresh?) {
    await this.cacheManager.set(
