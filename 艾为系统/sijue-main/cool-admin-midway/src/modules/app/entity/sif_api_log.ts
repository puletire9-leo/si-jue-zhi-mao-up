...
/**
 * SIF API 调用日志表
 * 记录每次 SIF 接口调用的详细信息，用于监控消耗和排查问题
 */
@Entity('app_sif_api_log')
export class AppSifApiLogEntity extends BaseEntity {
    ...
    @Index()
    @Column({ comment: 'SIF 接口路径', length: 200 })
    api_path: string;

    @Column({ comment: 'HTTP 方法 GET/POST', length: 10 })
    http_method: string;

    @Column({ comment: '关键词数量（如有）', type: 'int', default: 0 })
    keyword_count: number;

    @Column({ comment: '关键词内容（如有，最多存前5个）', type: 'text', nullable: true })
    ...
    @Column({ comment: '计费次数（=关键词数或ASIN数或1）', type: 'int', default: 1 })
    credit_count: number;

    @Column({ comment: '响应状态码（SIF code）', type: 'int', nullable: true })
