...
| 竞品分析 + 评分引擎 + ASIN 导入 + 筛选预设 + 领星对接 + 财务/运营自动化 | Java（sjzm-product） | ✅ |
...
9. **Maven 只在 Docker 里** — 宿主机没装 Maven，Java 编译走 `docker compose -f docker-compose.prod.yml build`，禁止在宿主机直接跑 `mvn`
