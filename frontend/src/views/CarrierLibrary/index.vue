...
const isAllSelected = computed(() => {
  return (
    carrierList.value.length > 0 &&
    selectedItems.value.length === carrierList.value.length
  );
});
...
const confirmFilter = (): void => {
  ...
  let message = "已应用筛选条件";
  if (
    filterCounts.developer > 0 ||
    filterCounts.status > 0 ||
    filterCounts.carrier > 0 ||
    filterCounts.batch > 0
  ) {
...
const handleDelete = async (carrier: Carrier): Promise<void> => {
  try {
    ...
    if (response.code === 200) {
      ...
      const index = carrierList.value.findIndex(
...
const handleBatchDownload = async (): Promise<void> => {
  ...
  try {
    // 收集所有选中的载体对象
    const selectedCarriers = carrierList.value.filter((carrier) =>
      selectedItems.value.includes(carrier.id),
    );
    ...
    for (const carrier of selectedCarriers) {
      ...
      if (carrier.images && carrier.images.length > 0) {
        for (let i = 0; i < carrier.images.length; i++) {
          ...
          const filename = formatFilename(
            `载体-${carrier.carrier_name}${i > 0 ? `-${i + 1}` : ""}.${ext}`,
          );
      ...
      if (carrier.reference_images && carrier.reference_images.length > 0) {
        for (let i = 0; i < carrier.reference_images.length; i++) {
          const originalUrl = ImageUrlUtil.getOriginalUrl(
            carrier.reference_images[i],
          );
          // 使用默认扩展名jpg，因为我们知道原始图片是jpg格式
          const ext = "jpg";
          const filename = formatFilename(
            `效果图-${carrier.carrier_name}${i > 0 ? `-${i + 1}` : ""}.${ext}`,
          );
          filesToDownload.push({ url: originalUrl, filename });
        }
      }
    ...
    // 显示开始下载的提示
    ElMessage.success(
      `开始批量下载，共 ${selectedCarriers.length} 个载体，${filesToDownload.length} 张图片`,
    );
    ...
    const downloadResponse = await fetch(
      `/api/v1/final-drafts/download-tasks/${taskId}/download`,
      {
        method: "GET",
        headers: {
          Authorization: token ? `Bearer ${token}` : "",
      ...
      while (true) {
        ...
        if (totalBytes > 0) {
          downloadProgress.value = Math.min(
            99,
            50 + Math.floor((receivedBytes / totalBytes) * 49),
          );
        }
      }
    }
    ...
    if (error.response) {
      if (error.response.status === 500) {
        ElMessage.error(
          `批量下载失败: 服务器内部错误，${error.response.data?.detail || "无法生成ZIP文件"}`,
        );
      } else if (error.response.status === 400) {
        ElMessage.error(
          `批量下载失败: 请求参数错误，${error.response.data?.detail || "无效的文件列表"}`,
        );
      } else {
...
const handleBatchDelete = async (): Promise<void> => {
  ...
  try {
    ...
    // 调用真实批量删除API
    const response = await carrierLibraryApi.batchDelete({
      ids: selectedItems.value,
    });

    if (response.code === 200) {
      ElMessage.success(
        `成功删除 ${response.data.success} 个载体，失败 ${response.data.failed} 个`,
      );
      const deletedIds = selectedItems.value;
      selectedItems.value = [];
      // 立即从列表中移除被删除的载体，确保UI立即更新
      if (deletedIds.length > 0) {
        carrierList.value = carrierList.value.filter(
          (carrier) => !deletedIds.includes(carrier.id),
        );
        // 如果是最后一页且删除后列表为空，切换到上一页
        if (carrierList.value.length === 0 && pagination.page > 1) {
          pagination.page--;
...
const handleDownload = async (carrier: Carrier): Promise<void> => {
  try {
    const totalImages =
      (carrier.images?.length || 0) + (carrier.reference_images?.length || 0);
    ...
    // 显示开始下载的提示
    ElMessage.success(
      `开始下载载体 ${carrier.carrier_name} 的图片，共 ${totalImages} 张`,
    );

    // 使用后端API进行下载
    const zipFilename = formatFilename(
      `载体-${carrier.carrier_name}-图片-${new Date().getTime()}.zip`,
    );
    ...
    if (reader) {
      ...
      ElMessage.success(
        `载体 ${carrier.carrier_name} 的图片已打包成ZIP文件下载完成`,
      );
    } else {
    ...
    ElMessage.error(
      `下载载体 ${carrier.carrier_name} 失败: ${error.message || "未知错误"}`,
    );
    downloadStatus.value = "exception";
    progressText.value = "下载失败！";
    setTimeout(() => {
...
const handleUpdateCarrier = async (updatedCarrier: Carrier): Promise<void> => {
  try {
    const response = await carrierLibraryApi.update(
      updatedCarrier.id,
      updatedCarrier,
    );
    if (response.code === 200) {
...
const loadSystemConfigs = async () => {
  try {
    // 开发人列表：直接读 users 表 role='开发' (统一入口)
    try {
      const members = await fetchMembers();
      developerOptions.value = members.developers || [];
    } catch (e) {
    ...
    if (
      carrierResponse.code === 200 &&
      carrierResponse.data &&
      Array.isArray(carrierResponse.data.carrierList)
    ) {
...
.drafts-grid {
  ...
  grid-template-columns: repeat(
    auto-fill,
    minmax(v-bind('cardWidth + "px"'), 1fr)
  );
  gap: 20px;
  margin-bottom: 0;
  min-height: 200px;
