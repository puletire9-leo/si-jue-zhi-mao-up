...
// 搜索类型选项 - 只保留元素搜索
const searchTypeOptions = [{ label: "元素", value: "element" }];
...
const handleDelete = async (draft: Draft): Promise<void> => {
  try {
    await ElMessageBox.confirm(`确定要删除素材 ${draft.sku} 吗？`, "提示", {
    ...
    if (draft.reference_images && draft.reference_images.length > 0) {
      for (let i = 0; i < draft.reference_images.length; i++) {
        const originalUrl = ImageUrlUtil.getOriginalUrl(
          draft.reference_images[i],
        );
        // 使用默认扩展名jpg，因为我们知道原始图片是jpg格式
        const ext = "jpg";
        const filename = formatFilename(
          `效果图-${draft.sku}${i > 0 ? `-${i + 1}` : ""}.${ext}`,
        );
        filesToDownload.push({ url: originalUrl, filename });
      }
    }
...
const handleBatchDelete = async (): Promise<void> => {
  ...
  try {
    ...
    // 调用真实批量删除API
    const response = await materialLibraryApi.batchDelete({
      ids: selectedItems.value,
    });

    if (response.code === 200) {
      ElMessage.success(
        `成功删除 ${response.data.success} 个素材，失败 ${response.data.failed} 个`,
      );
...
const confirmFileName = async (): Promise<void> => {
  ...
  if (isBatchDownload.value) {
    await handleBatchDownloadWithName(
      currentDownloadDrafts.value,
      fileNameForm.name,
      fileNameForm.includeDate,
    );
...
// 带文件名的单个下载方法
const handleDownloadWithName = async (
  draft: Draft,
  fileName: string,
): Promise<void> => {
  try {
    ...
    progressText.value =
      failedCount > 0 ? `任务创建完成，部分文件下载失败` : "任务创建成功！";
...
// 带文件名的批量下载方法
const handleBatchDownloadWithName = async (
  drafts: Draft[],
  fileName: string,
  includeDate: boolean,
): Promise<void> => {
  ...
  try {
    ...
    // 显示开始下载的提示
    ElMessage.info(
      `开始批量下载，共 ${drafts.length} 个素材，${filesToDownload.length} 张图片`,
    );
