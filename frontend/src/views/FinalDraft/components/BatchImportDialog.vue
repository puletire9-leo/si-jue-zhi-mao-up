<template>
  ...
  >
    <div class="batch-import-dialog">
        ...
        <div class="upload-item">
          ...
          >
            ...
            <div class="el-upload__tip">
              支持JPG、PNG格式，最大{{ imageSettings.maxImageSize }}MB
            </div>
          </el-upload>
        </div>
          ...
          <el-table-column label="操作">
            <template #default="{ row }">
              <el-button
                type="danger"
                size="small"
                @click="removeImportItem(row.index)"
              >
                <el-icon><Delete /></el-icon>
              </el-button>
            </template>
      ...
      >
        <template #format> {{ importProgress }}% {{ progressText }} </template>
      </el-progress>
    </div>
...
const importList = ref<
  Array<{
    ...
    status: "concept" | "optimizing" | "finalized";
    batch: string;
    developer: string;
    carrier: string;
    element: string;
    designImage?: { filename: string; url?: string; uid?: string };
    referenceImage?: { filename: string; url?: string; uid?: string };
  }>
>([]);
  ...
  if (currentUser) {
    ...
    if (isDeveloperRole) {
      ...
      if ((currentUser as any).developer) {
        ...
        console.log(
          "[BatchImport] 从用户对象的 developer 字段获取开发人:",
          (currentUser as any).developer,
        );
      }
      // 其次使用硬编码映射
...
const updateImportList = (): void => {
  ...
  importList.value = fileList.value.map((file, index) => {
    ...
    const existingItem = originalImportList.find((item) => item.uid === uid);
  ...
  importList.value = importList.value.map((item) => {
    ...
    if (!matchingReference) {
      matchingReference = referenceFileList.value.find((refFile) => {
        ...
        return (
          (lowerCaseRefSku.includes(lowerCaseItemSku) ||
            lowerCaseItemSku.includes(lowerCaseRefSku)) &&
          !matchedFiles.has(String(refFile.uid))
        );
      });
    }
...
// 处理侵权标注输入变化
const handleInfringementLabelInput = (
  row: {
    uid: string;
    index: number;
    sku: string;
...
const removeReferenceImage = (index: number) => {
  ...
  if (importList.value[index]) {
    ...
    if (referenceImage?.uid) {
      const fileIndex = referenceFileList.value.findIndex(
        (file) => String(file.uid) === referenceImage.uid,
      );
      if (fileIndex > -1) {
...
const handleReferenceImageClick = (index: number) => {
  ...
  if (referenceUploadRef.value) {
    (referenceUploadRef.value as any).$el
...
const uploadSingleImage = async (
  ...
  try {
    console.log(
      `开始上传单个图片: ${file.name}, 大小: ${(file.size / 1024 / 1024).toFixed(2)}MB, SKU: ${sku}`,
    );
    ...
    // 其他可能的成功响应格式
      ...
      ((response as any).url || (response.data as any)?.url)
    ) {
      success = true;
...
const uploadImages = async (
  ...
  try {
    ...
    console.log(
      `批量上传图片批次划分: 共 ${batches.length} 个批次，每批次 ${CONCURRENCY} 个文件`,
    );
    ...
    for (const batch of batches) {
      console.log(
        `开始上传批次 ${batchIndex}/${batches.length}，共 ${batch.length} 个文件`,
      );
      ...
      batchResults.forEach((result, index) => {
        ...
        if (result.status === "fulfilled") {
          ...
          if (result.value && typeof result.value === "object") {
            // 检查是否有有效的URL
            const url =
              result.value.url || result.value.cos_url || result.value.filepath;
            if (url) {
              results.push({ filename: fileName, url });
              console.log(
                `批次 ${batchIndex} 文件 ${index + 1} 上传成功: ${fileName}，SKU: ${sku}，URL: ${url}`,
              );
            } else {
              errors.push(fileName);
              console.error(
                `批次 ${batchIndex} 文件 ${index + 1} 上传失败: ${fileName}, SKU: ${sku}, 原因: 返回结果中没有有效的URL`,
              );
            }
          } else {
            errors.push(fileName);
            console.error(
              `批次 ${batchIndex} 文件 ${index + 1} 上传失败: ${fileName}, SKU: ${sku}, 原因: 返回结果无效`,
            );
          }
        } else {
          errors.push(fileName);
          console.error(
            `批次 ${batchIndex} 文件 ${index + 1} 上传失败: ${fileName}, SKU: ${sku}, 错误:`,
            result.reason,
          );
        }
      });
      ...
      console.log(
        `批量上传进度: ${uploadedCount}/${fileSkuPairs.length} (${progress}%)`,
      );
    }
    ...
    if (results.length === 0) {
      ...
      throw new Error(
        `批量上传失败: 没有文件上传成功，失败文件: ${errors.join(", ")}`,
      );
    }
    ...
    if (errors.length > 0) {
      console.log(
        `批量上传结果: 部分成功，成功 ${results.length} 个，失败 ${errors.length} 个，失败文件:`,
        errors,
      );
      ElMessage.warning(
        `图片上传完成: 成功 ${results.length} 个，失败 ${errors.length} 个`,
      );
    } else {
...
const handleImport = async (): Promise<void> => {
  ...
  try {
    ...
    for (const file of fileList.value) {
      if (file.raw) {
        // 查找对应文件的SKU信息
        const importItem = importList.value.find(
    ...
    console.log(
      `设计稿文件分离完成，共 ${designFileSkuPairs.length} 个文件需要上传`,
    );
    ...
    for (const item of importList.value) {
      if (item.referenceImage) {
        ...
        const matchingReferenceFile = referenceFileList.value.find(
          (refFile) => String(refFile.uid) === String(item.referenceImage?.uid),
        );
        if (matchingReferenceFile && matchingReferenceFile.raw) {
          referenceFileSkuPairs.push({
            file: matchingReferenceFile.raw,
            sku: item.sku,
          });
          referenceMatchCount++;
          console.log(
            `效果图匹配成功（通过uid）: ${item.referenceImage?.filename}，SKU: ${item.sku}`,
          );
        } else {
          // 如果uid匹配失败，尝试使用文件名匹配作为备选方案
          const altMatchingFile = referenceFileList.value.find(
            (refFile) => refFile.name === item.referenceImage?.filename,
          );
          if (altMatchingFile && altMatchingFile.raw) {
            referenceFileSkuPairs.push({
              file: altMatchingFile.raw,
              sku: item.sku,
            });
            referenceMatchCount++;
            console.log(
              `效果图匹配成功（通过文件名）: ${item.referenceImage?.filename}，SKU: ${item.sku}`,
            );
          } else {
            referenceFailCount++;
    ...
    console.log(
      `效果图文件分离完成，共 ${referenceFileSkuPairs.length} 个文件需要上传，匹配成功 ${referenceMatchCount} 个，失败 ${referenceFailCount} 个`,
    );
    ...
    console.log(
      `设计稿图片上传完成，成功 ${uploadedDesignUrls.length} 个，失败 ${designFileSkuPairs.length - uploadedDesignUrls.length} 个`,
    );
    ...
    if (referenceFileSkuPairs.length > 0) {
      console.log(
        `开始上传参考图图片，共 ${referenceFileSkuPairs.length} 个文件`,
      );
      progressText.value = "正在上传参考图...";
      uploadedReferenceUrls = await uploadImages(referenceFileSkuPairs);
      console.log(
        `参考图图片上传完成，成功 ${uploadedReferenceUrls.length} 个，失败 ${referenceFileSkuPairs.length - uploadedReferenceUrls.length} 个`,
      );
    ...
    const draftDataList = importList.value
      ...
      .map((item) => {
        ...
        console.log(
          `添加定稿数据: SKU=${item.sku}, 文件名=${item.filename}, 设计稿URL=${designUrl}, 参考图URL=${referenceUrl || "无"}`,
        );
        return {
    ...
    if (draftDataList.length === 0) {
      console.error(
        "没有可创建的定稿数据，上传成功的文件数:",
        uploadedDesignUrls.length,
        "导入列表长度:",
        importList.value.length,
      );
    ...
    if (createResponse.code === 200) {
      ...
      if (
        ...
        Array.isArray(createResponse.data.errors)
      ) {
