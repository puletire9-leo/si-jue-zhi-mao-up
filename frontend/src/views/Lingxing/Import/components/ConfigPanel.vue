...
/**
 * 从 /api/v1/auth/members（Python 无 RBAC 拦截）拉取默认人员。
 * 映射规则（与后端 lingxing.py 保持一致）：
 *   - 开发人      ← developers[0]
 *   - 产品负责人  ← operators 全部逗号拼接（"产品负责人"是领星 Excel 列名）
 *   - 采购员      ← purchasers[0]
 * 仅填充当前为空的项，不覆盖用户已存配置。
 */
const loadRosterDefaults = async () => {
  try {
    ...
    const { developers, operators, purchasers } = members

    if (developers.length) defaultConfig.developer = developers[0]
