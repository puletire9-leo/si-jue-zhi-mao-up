<template>
  ...
  <div v-else class="product-table-wrapper">
    ...
    >
      ...
      <el-table-column width="50" align="center" fixed="left">
        ...
        <template #default="{ $index }">
          ...
          >
            ...
            /></el-icon>
            <el-icon
              v-else-if="selectedRowIndex !== null"
              :size="18"
              color="#409eff"
              ><Pointer
            /></el-icon>
      ...
      <el-table-column prop="productName" label="产品名称" min-width="200">
        <template #default="{ row, $index }">
          <el-input
            v-if="
              editingCell.row === $index && editingCell.col === 'productName'
            "
      ...
      <el-table-column prop="cosImageUrl" label="图片URL" width="300">
        <template #default="{ row, $index }">
          <el-input
            v-if="
              editingCell.row === $index && editingCell.col === 'cosImageUrl'
            "
      ...
      <el-table-column prop="purchaseCost" label="采购费用" width="120">
        <template #default="{ row, $index }">
          <el-input-number
            v-if="
              editingCell.row === $index && editingCell.col === 'purchaseCost'
            "
      ...
      <el-table-column prop="supplierLink" label="供应商链接" width="150">
        <template #default="{ row, $index }">
          <el-input
            v-if="
              editingCell.row === $index && editingCell.col === 'supplierLink'
            "
      ...
      <el-table-column prop="ukCustomsCode" label="英国海关编码" width="150">
        <template #default="{ row, $index }">
          <el-input
            v-if="
              editingCell.row === $index && editingCell.col === 'ukCustomsCode'
            "
        ...
        <template #default="{ row, $index }">
          <el-input
            v-if="
              ...
              editingCell.col === 'cnDeclarationName'
            "
      ...
      <el-table-column prop="enDeclarationName" label="英文报关名" width="150">
        <template #default="{ row, $index }">
          <el-input
            v-if="
              editingCell.row === $index &&
              editingCell.col === 'enDeclarationName'
            "
    ...
    <div
      ...
      :class="{
        'scroll-up': scrollIndicator.direction === 'up',
        'scroll-down': scrollIndicator.direction === 'down',
      }"
    >
      <el-icon :size="32">
    ...
    <div class="drag-hint">
      ...
      <span
        >提示：点击左侧 ⇅ 图标选择行，再点击另一行的 ⇅
        图标即可交换两者的图片、文件码、图片URL信息</span
      >
    </div>
...
const rowClassName = (data: { row: any }) => {
  const sku = (data.row?.sku || "").trim();
  if (!sku) return "row-sku-missing";
  return "";
};
...
const handleRowSelect = (index: number) => {
  ...
  ElMessage.success(
    `已交换第${sourceIndex + 1}行和第${targetIndex + 1}行的图片信息`,
  );
...
const handleGlobalDragOver = (event: DragEvent) => {
  ...
  const tableWrapper = document.querySelector(
    ".product-table .el-table__body-wrapper",
  ) as HTMLElement;
