...
export function getProductLineModel(nodeId: number, marketplace: string, batchId?: string) {
  const params: Record<string, any> = { marketplace }
  if (batchId) params.batchId = batchId
  return request.get(`${MODEL_BASE}/model/${nodeId}`, { params })
}

export function getProductLineElements(nodeId: number, marketplace: string, month?: string) {
  return request.get(`${MODEL_BASE}/elements`, { params: { node_id: nodeId, marketplace, month } })
}
