...
export const systemConfigApi = {
   ...
   */
  async startBackup(
    backupType: string = "local",
  ): Promise<ApiResponse<BackupStartResponse>> {
    try {
   ...
   * 更新载体列表
   */
  async updateCarrierList(
    carrierList: string[],
  ): Promise<ApiResponse<CarrierListResponse>> {
    try {
      // 过滤空字符串，确保数组有效
  ...
  ): Promise<ApiResponse<ImageSettingsResponse>> {
    try {
      const response = await request({
        url: "/api/v1/system-config/image-settings",
        method: "put",
   ...
   * 获取卖家精灵配置
   */
  async getSellerspriteConfig(): Promise<
    ApiResponse<SellerspriteConfigResponse>
  > {
    try {
      const response = await request({
