...
/** 研发可见但屏蔽的管理后台 */
const DEVELOPER_HIDDEN = new Set(["users", "settings"]);

/**
 * 判断角色是否能看到指定模块（用于侧边栏菜单过滤）
 * @param role 用户角色（中文，支持逗号分隔多角色如 "管理员,运营"）
 ...
 * 管理员 和 开发 可以写（兼容多角色逗号分隔）
 */
export function canWrite(role: string | undefined | null): boolean {
  if (!role) return false;
  return (
    role.includes("管理员") || role.includes("admin") || role.includes("开发")
  );
}
