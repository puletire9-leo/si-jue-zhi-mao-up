...
只更新受影响的 Compose 服务。虽然 Java 三个服务共用镜像，且当前 Dockerfile 在镜像构建阶段会编译
gateway/user/product 三个模块，但只改 product 时仍只执行 `up -d --no-build java-product`，
不得顺手重建或重启 `java-user`、`gateway`；其他模块同理。
