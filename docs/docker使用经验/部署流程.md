...
> **生产部署唯一权威文件。** 所有人员和 Agent 在生产构建、更新、回滚、清理前必须完整读取本文件。日志、问题记录、OpenSpec 实施记录和旧架构文档中的命令只代表历史上下文，不得直接执行。
> 先看目录再动手：[README.md](README.md)（现网端口、容器、该打开哪份文档）。
>
> 现在有两套生产，命令不能混用。
...
| | 线下 | 线上 |
|---|---|---|
| 在哪 | 本机 Windows Docker，入口 `http://localhost:5173` | 对方服务器 `/root/woeau_web/ai-selection-deploy` |
| 编排 | 仓库根 `docker-compose.prod.yml` | 该目录 `docker-compose.yml`（来自 `deploy-remote/`） |
| Web 容器 | `prod-frontend` | 同样是 `prod-frontend`（服务名 `frontend`） |
| 日常更新 | 只改一个组件，见 [日常更新docker.md](日常更新docker.md) | 推镜像 → 改 `.env` 里的镜像变量 → `docker compose up -d` |
| 禁止 | 直接 `up -d --build`、`--no-cache`、加 `-p` | 在本机跑线上命令；把 SQL 灌进 RDS |

线下项目名必须是目录默认值 `si-jue-zhi-mao-up`。**不要加 `-p sijuelishi-prod`**。

## 线下生产发布入口

日常**线下**发布只允许从仓库根目录运行统一脚本：
...
每次更新只执行与源码变更对应的一条命令。Java、前端、Python 后端和 AI Center 彼此独立轮换，
不会因为发布其中一个组件而重新构建其他镜像。统一脚本对受影响服务使用 `--no-deps --no-build`，
因此不会联动重建 MySQL、Redis、Nacos 或其他未变更服务。除非确实修改了对应源码，否则禁止把四条
组件命令串联执行；这样会浪费构建缓存、增加磁盘临时空间和生产重启次数。

| 本次变更 | 执行命令 |
|---|---|
| Java user/product/gateway | `powershell -ExecutionPolicy Bypass -File scripts/deploy/deploy_prod.ps1 -Component java` |
| Vue 前端或 Nginx 配置 | `powershell -ExecutionPolicy Bypass -File scripts/deploy/deploy_prod.ps1 -Component frontend` |
| Python API/Celery | `powershell -ExecutionPolicy Bypass -File scripts/deploy/deploy_prod.ps1 -Component backend` |
| AI Center | `powershell -ExecutionPolicy Bypass -File scripts/deploy/deploy_prod.ps1 -Component ai-center` |

脚本强制执行：预检（磁盘空间、BuildKit builder、底层 snapshot 完整性） -> 删除更老的 `previous` -> 把当时的 `current` 收成唯一 `previous` -> 单次缓存构建 ->
`--no-build` 重建受影响服务 -> 运行状态验证 -> **保留这一份 `previous` 作回退**，并删掉该仓库其它多余标签 -> Java 编译缓存只保留最新一条 -> 强制清理超过 3 小时未使用的普通构建层。
缓存完整性异常必须在镜像轮换和 Maven 执行前停止，不允许一边重新下载依赖一边继续发布。任何步骤失败立即停止，不得跳到后续步骤。脚本不删除 volume，不执行 `docker system prune` 或无过滤条件的
BuildKit 全量清理。只有排障时才允许手工执行本文件后续分解命令，并必须记录原因和结果。

## 线上生产更新

目录：`/root/woeau_web/ai-selection-deploy`（仓库里的 `deploy-remote/` 拷过去的）。  
Web 容器就是 `frontend` / `prod-frontend`。对方 Nginx 反代 `127.0.0.1:5173`。
正式入口为 `https://selection.suezon.com/ai-selection`；域名解析和 SSL 证书已经在服务器外层 Nginx 配置完成。普通业务镜像更新不需要重新申请或配置证书；只有域名、证书文件或外层 Nginx 配置本身变化时才单独处理。

容器名不用改，改的是 `.env` 里的**镜像变量**：

```text
FRONTEND_IMAGE=registry.cn-shanghai.aliyuncs.com/suezon/selection:frontend-v1.0
JAVA_IMAGE=registry.cn-shanghai.aliyuncs.com/suezon/selection:java-v1.0
BACKEND_IMAGE=registry.cn-shanghai.aliyuncs.com/suezon/selection:backend-v1.0
AI_CENTER_IMAGE=registry.cn-shanghai.aliyuncs.com/suezon/selection:ai-center-v1.0
```

| 改了什么 | 推哪个 tag | 改 `.env` 哪一行 |
|---|---|---|
| 前端 | `frontend-v1.1` | `FRONTEND_IMAGE` |
| Java（user/product/gateway 共用） | `java-v1.1` | `JAVA_IMAGE` |
| Python / Celery | `backend-v1.1` | `BACKEND_IMAGE` |
| AI Center | `ai-center-v1.1` | `AI_CENTER_IMAGE` |

本机先发布再推仓：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/deploy/deploy_prod.ps1 -Component frontend
docker tag prod-frontend:current registry.cn-shanghai.aliyuncs.com/suezon/selection:frontend-v1.1
docker push registry.cn-shanghai.aliyuncs.com/suezon/selection:frontend-v1.1
```
...
# 编辑 .env，把对应 IMAGE 改成新 tag
docker compose up -d
```

`up -d` 会按新 tag 拉镜像并重启对应容器。不要四个镜像每次全推。不要在线上对业务镜像 `--build`。  
首次部署、灌库、RDS 说明见 [deploy-remote/README.md](../../deploy-remote/README.md)。
...
6. 发布验证后，Java 精确保留最新一条源码编译记录；其他 `regular` 构建层只保留最近 3 小时内使用的记录，超过 3 小时必须删除；Maven/pip/npm 的 `exec.cachemount` 和基础镜像热缓存继续保留。
7. Java 生产构建由统一脚本显式传入 `MAVEN_OFFLINE=true`；缺少依赖时直接失败。只有 POM 新增依赖且负责人明确批准联网后，才单独执行 Maven 缓存预热脚本。

“快速读取缓存构建”指复用依赖层和未变化的 COPY 层，不是永久保留全部历史源码编译结果。热依赖缓存保障下次构建速度，旧 Java `mvn clean package` 结果在发布验证后只保留最新一条。

## Docker 存储控制

详细巡检、分级清理、受保护卷清单和 VHDX 物理压缩步骤见
[Docker存储优化.md](Docker存储优化.md)。本节只保留发布流程必须执行的存储规则。
...
生产成品镜像每个仓库常态只保留 `current` 加一份 `previous` 回退。构建前先丢掉更老的回退件，再把原 `current` 标成唯一 `previous`，然后写入新的 `current`。验证成功后不得再删这份 `previous`，也不得留下 `latest` 或第三份成品。日常操作见 [日常更新docker.md](日常更新docker.md)。

BuildKit 缓存与成品镜像不是同一概念。Maven、pip、基础镜像等通用依赖缓存必须保留；
历史源码编译结果、旧 JAR 和旧应用 COPY 层按最后使用时间清理。Maven/pip/npm 的
`exec.cachemount` 不参与日常清理。

> **强制存储纪律：禁止重复构建制造垃圾。** 同一组件、同一份源码在一次任务中最多正常构建一次；
> 构建失败必须先修复根因，禁止原命令盲目重跑。普通 `regular` 构建层只保留最近 3 小时，
> 超过 3 小时未使用的记录必须在每次成功发布后自动回收，不得因“以后可能用到”长期堆积。
...
成品镜像与 BuildKit 缓存不是同一层：成品常态保留 `current` 加一份 `previous`；BuildKit 继续保留近期命中的通用依赖层，只按时间删除历史源码编译层。

Java 是额外的强制例外：`Dockerfile.prod` 的每次构建都会产生一条约 460 MB 的
`mvn clean package` 编译缓存。该类记录常态只允许保留最新 1 条；发布验证完成后必须立即删除所有更旧记录。

禁止人工执行无过滤条件的 `docker buildx prune --all`。统一运行专用脚本，脚本会冻结并预览完整缓存
ID 集合，只对描述匹配本项目 Java 编译命令的旧记录使用锚定 ID 正则，并强制保护最新一条：
...
生产表结构使用 `MYSQL_USER/MYSQL_PASSWORD` 业务账号只读检查；binlog 保留期、总量和复制通道
使用 `config/secrets/prod.env` 的 `MYSQL_ROOT_PASSWORD` 只读检查。业务账号通常无权读取
`performance_schema.replication_connection_configuration`，不得因此放宽权限或跳过预检。

`backend/.dockerignore` 必须排除 `database/`、`backup/`、`uploads/`、`images/` 和下载缓存。
这些目录包含本地 SQL 备份或运行数据，不是镜像源码；纳入构建上下文会让一次 Python 构建额外
传输约 1 GB，并把历史备份烘焙进不可变镜像层。忽略规则只影响构建，不删除宿主文件。
...
不同入口（例如 `localhost:5173` 与线上域名）是不同 localStorage 域，token 互相看不见，换入口必须重新登录。
...
### 14. 禁止高频 `docker exec` 打爆 WSL2

分析数据时不要循环 `docker exec prod-mysql mysql -e "..."`，也不要把这类脚本后台并发。WSL2 内存约 9GB，打满后 Docker daemon 会卡死。一次连接跑完所有查询：Python `pymysql` 直连 `127.0.0.1:3310`，或把多条 SQL 拼进一次 `docker exec`。崩了先重启 Docker Desktop，无效再 `wsl --shutdown`。
...
生产 Terser 固定只使用 1 个 worker，避免按 CPU 数默认并发压缩时挤占生产容器所需内存。
日常前端镜像构建保留 BuildKit 缓存；`nginx:alpine` 和未变化的 COPY 层会自动复用。
生产发布禁止默认使用 `--no-cache`；统一预检会通过 `docker system df -v` 核验镜像存储，并通过
`docker buildx inspect --bootstrap`、`docker buildx du` 核验活动 BuildKit。仅全局镜像统计发现未被活动
BuildKit 使用的孤立 snapshot 记录时告警继续；活动 BuildKit 缺失 snapshot、I/O error 或只读文件系统
必须在构建前停止并单独修复 Docker 存储。
...
> 只用于空机、镜像被清光、或第一次把整套栈拉起来。栈已经在跑时不要走本节全量 `compose build`，改走 [日常更新docker.md](日常更新docker.md)。
...
日常单组件更新、构建前清旧构件、以及「只留一份 previous 回退」的完整说明见 **[日常更新docker.md](日常更新docker.md)**。下面命令表仍是权威抄写源，禁止从日志里另抄一套。

### 生产镜像 current + 一份 previous

应用成品镜像每个仓库常态保留两个标签：`prod-java` / `prod-backend` / `prod-frontend` / `prod-ai-center` 的 `current` 与 `previous`。MySQL、Redis、Nacos 数据卷不受此规则影响。

```text
发布前：current(N)
构建前：删掉更老的 previous；把 current(N) 标成唯一 previous
构建中：previous(N) + current(N+1)
验证失败：用 previous 回退
验证成功：继续保留 previous(N)，只删该仓库其它多余标签
```

更老的回退镜像仍被容器引用时必须停止清理并查明原因，禁止强制删除。Java `mvn clean package` 编译缓存仍只保留最新一条。

统一发布脚本已经实现上述轮换和多余标签清理，禁止手工复制其中的分解命令。普通构建层固定使用带 `until=3h`、`type=regular` 的 BuildKit 清理；禁止删除 volume 或依赖 cache mount。
...
> AI Center 独立使用 `prod-ai-center:current`，不得与 `prod-backend` 混合轮换；该仓库同样只留一份 `previous` 作回退。
...
```powershell
# Nginx 配置在 frontend/nginx.conf，仍执行前端成品 current + 一份 previous 发布流程
Start-ProdImageRotation -Repository "prod-frontend"
docker compose -f docker-compose.prod.yml build frontend
docker compose -f docker-compose.prod.yml up -d --no-deps --no-build frontend
```
...
- 成品镜像每个仓库常态保留 `current` 加一份 `previous`；更老的成品和多余标签必须删掉
- 发布后核对每个组件只有 `current` 与 `previous`（该仓库第一次发布可以只有 `current`），并确认没有无标签旧成品
- BuildKit 的 `regular` 构建层只保留最近 3 小时，超过 3 小时未使用的记录由发布脚本自动清理；`exec.cachemount` 热依赖仓库不参与自动清理
- Java `mvn clean package` 编译缓存使用专用脚本严格只保留最新 1 条
...
- Maven 本地仓库使用固定 ID `sjzm-maven-repository` 的 BuildKit `cache mount`，`dependency:go-offline` 与 `clean package` 共用同一仓库
- **pom 没变 = POM/COPY 层命中，Maven 仓库直接复用**，只重编源码（几十秒）
- **pom 变了且新增依赖 = 正式发布离线失败**，不会在部署中自行下载；负责人批准后先运行下方预热脚本，再重新走统一发布
...
- Java 编译缓存常态最多 1 条；新构建验证成功后必须运行
  `scripts/deploy/prune_java_build_cache.ps1` 删除全部更旧编译记录。
- 只用新镜像重建实际受影响的服务，命令带 `--no-build`，避免 `up` 阶段再次隐式构建
- 构建带进度：`--progress=plain`

只有新增 Maven 依赖时允许使用以下独立入口；`-AllowNetworkDownload` 表示负责人已经明确批准本次联网下载：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/deploy/prepare_java_maven_cache.ps1 -AllowNetworkDownload
```

该脚本只构建到 `dependency-base`，填充 `sjzm-maven-repository`，不生成生产成品镜像、不轮换
`current`、不重启任何服务。未提供授权开关时必须直接拒绝运行。预热成功后，正式发布仍只能运行
`deploy_prod.ps1 -Component java`。
...
# 清理超过 3 小时未使用的普通构建层，保留 Maven/pip/npm cache mount
docker buildx prune --force --filter "until=3h" --filter "type=regular"
```
...
**长期修复（2026-08-17）**: `frontend/nginx.conf` 中所有可能重建的容器上游都必须通过变量配合
Docker DNS `resolver 127.0.0.11 valid=10s` 动态解析。`backend` 和 `gateway` 均使用
`$backend_upstream` / `$gateway_upstream`，禁止恢复为静态 `proxy_pass http://gateway:9000`；否则
Java 统一发布重建 Gateway 后，前端仍缓存旧容器 IP，`/api/v1/users`、`/api/v1/settings` 等接口会返回 502。
