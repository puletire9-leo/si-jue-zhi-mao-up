...
| 日常更新 | `scripts/deploy/deploy_prod.ps1 -Component ...` | 推镜像 → 改 `.env` 里的镜像变量 → `docker compose up -d` |
...
脚本强制执行：预检（磁盘空间、BuildKit builder、底层 snapshot 完整性） -> 删除最老 `previous` -> `current` 转为 `previous` -> 单次缓存构建 ->
`--no-build` 重建受影响服务 -> 运行状态验证 -> 删除临时回退镜像 -> Java 编译缓存只保留最新一条 -> 强制清理超过 3 小时未使用的普通构建层。
...
生产成品镜像常态只保留 `current`。构建前临时把原 `current` 标记为 `previous`，仅用于本次发布失败时回退；新版本健康验证通过后必须删除临时 `previous` 标签及其无引用镜像本体。
...
成品镜像与 BuildKit 缓存不是同一层：成品常态只保留 `current`；BuildKit 继续保留近期命中的通用依赖层，只按时间删除历史源码编译层。
...
应用成品镜像常态只保留一个 `current`：`prod-java:current`、`prod-backend:current`、`prod-frontend:current`、`prod-ai-center:current`。MySQL、Redis、Nacos 数据卷不受此规则影响。

发布过程允许临时存在 `previous`：

```text
发布前：current(N)
构建前：把 current(N) 临时标记为 previous(N)
构建中：previous(N) + current(N+1)
验证失败：使用临时 previous 回退
验证成功：删除 previous 标签及其无引用镜像本体，只留 current(N+1)
```

临时回退镜像仍被容器引用时必须停止清理并查明原因，禁止强制删除。发布完成后每个组件只能显示一个 `current` 标签；Java `mvn clean package` 编译缓存也只保留最新一条。

统一发布脚本已经实现上述临时回退和成功后清理，禁止手工复制其中的分解命令。普通构建层固定使用带 `until=3h`、`type=regular` 的 BuildKit 清理；禁止删除 volume 或依赖 cache mount。
...
> AI Center 独立使用 `prod-ai-center:current`，不得与 `prod-backend` 混合轮换；发布期间的临时 `previous` 由统一脚本管理并在成功后删除。
...
```powershell
# Nginx 配置在 frontend/nginx.conf，仍执行前端成品单版本发布流程
Start-ProdImageRotation -Repository "prod-frontend"
docker compose -f docker-compose.prod.yml build frontend
docker compose -f docker-compose.prod.yml up -d --no-deps --no-build frontend
```
...
- 成品镜像常态只保留 `current`；`previous` 只在单次发布期间临时存在，验证成功后立即删除
- 发布后核对每个组件只剩一个 `current`，并确认没有该组件的无标签旧成品
