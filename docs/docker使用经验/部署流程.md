...
初始化只新增标签，不重建容器、不删除镜像。不存在 `latest` 的新组件（例如首次发布 AI Center）
不得凭空初始化标签，直接运行统一发布脚本；脚本会先构建 `current`，健康验证通过后建立首次
`previous` 基线。首次基线的两个标签可指向同一镜像，下一次成功发布后即成为两个不同版本。
确认 `current` 标签正确后，后续发布全部走双版本流程。
