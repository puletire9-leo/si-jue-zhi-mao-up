...
ON DUPLICATE KEY UPDATE display_name=VALUES(display_name), weight=VALUES(weight), thresholds=VALUES(thresholds);
...
-- 5. 验证数据插入成功
SELECT '评分维度配置' as table_name, COUNT(*) as count FROM scoring_config
UNION ALL
SELECT '等级阈值配置', COUNT(*) FROM grade_thresholds;
