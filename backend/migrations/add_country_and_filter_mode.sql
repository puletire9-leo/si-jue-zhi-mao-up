...
SET @sql = IF(@index_exists = 0, @sql, 'SELECT "idx_country 索引已存在，跳过创建"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @indexname = 'idx_data_filter_mode';
...
SET @sql = IF(@index_exists = 0, @sql, 'SELECT "idx_data_filter_mode 索引已存在，跳过创建"');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SELECT '迁移完成：已成功添加 country 和 data_filter_mode 字段' AS message;
